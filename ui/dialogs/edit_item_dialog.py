"""
Edit Item Dialog - For editing items in cargo/attachments blocks
"""
...
from models.spawnable_type import SpawnableItem

class EditItemDialog(QDialog):
    def __init__(self, parent, item: SpawnableItem, types_files: List[TypesFile], 
                 has_multiple_items: bool):
        super().__init__(parent)
        self.item = item
        self.types_files = types_files
        self.has_multiple_items = has_multiple_items
    ...
    def init_ui(self):
        ...
        self.item_combo.setCurrentText(self.item.name)
        ...
        # Set current chance
        if self.item.chance is not None:
    ...
    def get_item_chance(self) -> Optional[float]:
        ...
        # If this is the only item and chance is 1.0, return None (implicit)
        if not self.has_multiple_items and chance == 1.0:
            return None
