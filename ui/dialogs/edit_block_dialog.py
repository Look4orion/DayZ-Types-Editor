"""
Edit Block Dialog - For editing existing cargo/attachments blocks
"""
...
from models.spawnable_type import CargoBlock, AttachmentsBlock
from typing import Optional, Union

class EditBlockDialog(QDialog):
    def __init__(self, parent, block: Union[CargoBlock, AttachmentsBlock], 
                 block_kind: str, random_presets_file: Optional[RandomPresetsFile]):
        super().__init__(parent)
        self.block = block
        self.block_kind = block_kind  # "cargo" or "attachments"
        self.random_presets_file = random_presets_file
        
        self.setWindowTitle(f"Edit {block_kind.title()} Block")
        self.setModal(True)
    ...
    def init_ui(self):
        ...
        # Set initial state based on current block
        if self.block.is_preset_based():
            self.preset_radio.setChecked(True)
        else:
            self.chance_radio.setChecked(True)
        
        type_group.setLayout(type_layout)
        layout.addWidget(type_group)
        
        # Warning label
        self.warning_label = QLabel("⚠ Changing block type will clear all items!")
        ...
        # Set current values
        if self.block.is_preset_based():
            # Find and select current preset
            index = self.preset_combo.findText(self.block.preset)
            if index >= 0:
                self.preset_combo.setCurrentIndex(index)
        else:
            self.chance_spin.setValue(self.block.chance)
    ...
    def on_type_changed(self):
        ...
        # Show warning if changing type
        type_changed = (is_preset and not self.block.is_preset_based()) or \
                      (not is_preset and self.block.is_preset_based())
        
        self.warning_label.setVisible(type_changed)
    
    def validate_and_accept(self):
        if self.preset_radio.isChecked():
            if not self.random_presets_file:
                QMessageBox.warning(self, "No Presets", 
                                  "Random presets file is not loaded. Cannot use preset-based block.")
                return
            
            if self.preset_combo.count() == 0 or self.preset_combo.currentText().startswith("("):
                QMessageBox.warning(self, "No Preset", 
                                  f"No {self.block_kind} presets available.")
                return
        
        # Confirm if changing type
        type_changed = (self.preset_radio.isChecked() and not self.block.is_preset_based()) or \
                      (self.chance_radio.isChecked() and self.block.is_preset_based())
        
        if type_changed:
            reply = QMessageBox.question(
                self,
                "Confirm Type Change",
                "Changing the block type will remove all existing items. Continue?",
